...
REGISTRY_IMPL=$(cast implementation "$PLATFORM_REGISTRY_ADDRESS" --rpc-url "$POLYGON_RPC_URL")
WALLET_BEACON=$(cast call "$PLATFORM_REGISTRY_ADDRESS" "walletBeacon()(address)" --rpc-url "$POLYGON_RPC_URL")
WALLET_IMPL=$(cast call "$WALLET_BEACON" "implementation()(address)" --rpc-url "$POLYGON_RPC_URL")
