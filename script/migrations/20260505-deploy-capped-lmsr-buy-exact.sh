#!/usr/bin/env bash
#
# Deploy a fresh CappedLMSRDeterministicFactory whose constructor auto-deploys a
# fresh CappedLMSRMarketMaker implementation. The new implementation is
# source-identical to the previous one EXCEPT it inherits the APA-455 buy-exact
# entry points (`calcBuyExactCollateral`, `buyExactCollateral`) via the pure
# `LMSRBuyExactMath` library. All math comes from the library, no new storage,
# no audited-parent edits.
#
...
# - Deploys the factory with `forge create --libraries`.
# - Records the new factory address as `cappedLmsrFactory` in
#   script/output/<chainId>.json. The previous factory address is preserved as
#   `cappedLmsrFactoryLegacy` for audit trail; nothing reads that field.
...
FACTORY_CONTRACT_PATH="src_market_ext/CappedLMSRDeterministicFactory.sol:CappedLMSRDeterministicFactory"
MIGRATION_NAME="$(basename "$0" .sh)"

# --- Read linked-lib address + previous factory ------------------------------
...
PREVIOUS_FACTORY_ADDRESS=$(jq -r '.cappedLmsrFactory // empty' "$OUTPUT_PATH")
if [[ -z "$PREVIOUS_FACTORY_ADDRESS" || "$PREVIOUS_FACTORY_ADDRESS" == "null" ]]; then
  echo "ERROR: cappedLmsrFactory not found in $OUTPUT_PATH"
  echo "       Run the initial chain bootstrap first."
  exit 1
fi
echo "  Previous cappedLmsrFactory: $PREVIOUS_FACTORY_ADDRESS"
...
echo "  Deploying CappedLMSRDeterministicFactory (auto-deploys impl in constructor)..."
FOUNDRY_PROFILE=market_ext forge create \
  --rpc-url "$RPC_URL" \
  --private-key "$PRIVATE_KEY" \
  --broadcast \
  --libraries "${FIXED_MATH_LIB_PATH}:${FIXED_MATH_LIB_ADDRESS}" \
  "$FACTORY_CONTRACT_PATH" | tee "$DEPLOY_LOG"

FACTORY_ADDRESS=$(grep "Deployed to:" "$DEPLOY_LOG" | sed 's/.*Deployed to: //' | tr -d '[:space:]')
TX_HASH=$(grep "Transaction hash:" "$DEPLOY_LOG" | sed 's/.*Transaction hash: //' | tr -d '[:space:]')

if [[ -z "$FACTORY_ADDRESS" || "$FACTORY_ADDRESS" != 0x* ]]; then
  echo "ERROR: failed to parse deployed address from forge output"
  exit 1
fi
echo "  CappedLMSRDeterministicFactory deployed at: $FACTORY_ADDRESS"

# Read out the implementation master that the factory's constructor just deployed,
# so it shows up in operator logs (audit trail; no file writes from this lookup).
IMPL_ADDRESS=$(cast call --rpc-url "$RPC_URL" "$FACTORY_ADDRESS" 'implementationMaster()(address)' 2>/dev/null || true)
if [[ -n "$IMPL_ADDRESS" && "$IMPL_ADDRESS" == 0x* ]]; then
  echo "  implementationMaster: $IMPL_ADDRESS"
fi

# --- Record addresses --------------------------------------------------------
jq \
  --arg new "$FACTORY_ADDRESS" \
  --arg old "$PREVIOUS_FACTORY_ADDRESS" \
  '. + {cappedLmsrFactory: $new, cappedLmsrFactoryLegacy: $old}' \
